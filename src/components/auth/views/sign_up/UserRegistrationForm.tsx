    ...
    if (form.photo.value && !form.photo.message) {
      let photoRef = await toast.promise(
        uploadFileBase64(DirectoryPath.Users, form.photo.value),
        {
          ...
          error:
            "Error al subir la foto de perfil, inténtalo de nuevo por favor",
        },
      );
      if (photoRef) {
        newUserData = {
        ...
        if (amountOfUsers > 0) {
          ...
          if (userId) {
            await toast.promise(createData(userId, fakeId), {
              ...
              success: `Usuario ${USER_ROLE_TO_SPANISH[form.role]} creado`,
              error:
                "Error al crear el nuevo usuario, inténtalo de nuevo por favor",
            });
          } else {
            setFormState({
              ...formState,
      ...
      >
        ...
        <TextField
          field={{
            ...
            setter: (e) => setForm((prev) => ({ ...prev, fullName: e })),
        ...
        <ImageUploader
          ...
          uploader={{
            image: form.photo,
            setImage: (e) => setForm((prev) => ({ ...prev, photo: e })),
          }}
        />
      </BaseForm>
    </div>
