...
const ConsentForm: React.FC<Props> = ({ id, moduleTarget, children }) => {
    ...
    if ((validReason || validId) && user && user.id) {
      ...
      try {
        await toast.promise(saveReq(infoDoc), {
          ...
          error:
            "Error al guardar el justificativo, inténtalo de nuevo por favor",
        });
        setAbleToSee({
          isChecking: false,
  ...
  const isValidID = useCallback(() => {
    return (
      form.complaintId.value.trim().length > 0 && !form.complaintId.message
    );
  }, [form.complaintId]);

  const isAtLeastOneValid = useCallback((): boolean => {
    return isValidReason() || isValidID();
  }, [isValidReason, isValidID]);
  ...
  }, [checkingUserAuth, id, moduleTarget, user]);
        ...
        <div className="margin-bottom-25">
          <div>
            ...
            <p className="text | light">
              Necesitas escribir una justificación para ver esta información,
              por favor llena uno o ambos de los siguientes campos.
            </p>
            <p className="text | light">
              <b>
                Podrás ver esta información hasta el final del dia de hoy o
                hasta salgas de tu cuenta
              </b>
            </p>
          </div>
          ...
          >
            <TextField
              field={{
                values: form.reason,
                setter: (d) => setForm((prev) => ({ ...prev, reason: d })),
                validator: isValidChangeReason,
              }}
              legend={`Justificación ${isValidID() ? "(Opcional)" : ""}`}
            />
            <TextField
              field={{
              ...
              legend={`ID de queja ${isValidReason() ? "(Opcional)" : ""}`}
            />
          </BaseFormWithTwoButtons>
        </div>
      </Popup>
