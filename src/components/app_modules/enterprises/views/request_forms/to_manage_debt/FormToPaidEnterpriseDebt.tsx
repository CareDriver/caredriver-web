...
const FormToPaidEnterpriseDebt: React.FC<Props> = ({ enterprise }) => {
    ...
    const paidDebt = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            await toast.promise(
                updateEnterprise(enterprise.id, enterpriseUpdated),
                {
                    pending: "Guardando pago en el historial",
                    success: "Pago guardado en el historial",
                    error: "Error al guardar el pago en el historial, intentalo de nuevo por favor",
                },
            );
            ...
            >
                <TextField
                    field={{
                        ...
                        setter: (b) =>
                            setForm((prev) => ({ ...prev, paidDebt: b })),
                        validator: (paidDebt) =>
                            isValidDebtPaid(
                                enterprise.currentDebt?.amount ?? 0,
                                paidDebt,
                            ),
                    }}
            ...
            <Popup isOpen={isOpenPopup} close={() => setOpenPopup(false)}>
                <div>
                    ...
                    <p className="text | light">
                        ...
                        bancaria para justificar el pago de la deuda.
                    </p>
                </div>
                <BaseForm
                    content={{
