...
import {
    ...
    isValidForm,
    verifyNoEmptyData,
} from "@/components/app_modules/enterprises/validators/EnterpriseValidator";
...
const LaundryRegistration = () => {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const { isValid, message } = validateEnterpriseName(value);
    ...
    useEffect(() => {
        if (!loadingUser) {
            if (user.data && user.data.id) {
                countEnterprisesActives(user.data.id, "laundry").then((r) => {
                    if (r >= MAX_NUMBER_ENTERPRISES) {
                        toast.error(
                            "Alcanzaste el limite de lavaderos que puedes registrar",
                        );
    ...
    return !loadingUser && !isCheckingAvailability ? (
        <section className="service-form-wrapper">
            ...
            <p className="text | light">
                Necesitamos verificar que el nuevo lavadoro de vehículos sea valido antes
                de registrarlo.
            </p>
            <form
