...
const FormToDeleteUserFromEnterprise: React.FC<Props> = ({
      ...
      if (requestsToRefuse.length > 0) {
        requestsToRefuse.forEach(async (request) => {
          if (request && user && user.id) {
            await saveReview(request, user.id, false, driveReqCollection);
          }
        });
      }
    }
  ...
  const refuseUserLaundryRequest = async (userWithRequests: UserInterface) => {
    ...
    if (
      ...
      if (requestToRefuse) {
        await saveReview(requestToRefuse, user.id, false, laundryReqCollection);
      }
    }
  };
  ...
  const refuseUserTowRequest = async (userWithRequest: UserInterface) => {
    ...
    if (
      ...
      if (requestToRefuse) {
        await saveReview(requestToRefuse, user.id, false, towReqCollection);
      }
    }
  };
      ...
      if (
        ...
        if (selectedUser.role === "user") {
          await toast.promise(refuseUserRequests(selectedUser.data), {
            ...
            error:
              "Error al eliminar las peticiones activas, inténtalo de nuevo",
          });
        }
        ...
        if (enterprise.type === "driver" && selectedUser.role === "user") {
          haveToUpdate = true;
          ...
          let serviceRequestsUpdated: ServiceRequestsInterface = selectedUser
          ...
          newUserData = {
            ...
            services: selectedUser.data.services
              ? selectedUser.data.services.filter((s) => s !== Services.Driver)
              : [],
        ...
        if (haveToUpdate) {
          await toast.promise(updateUser(selectedUser.data.id, newUserData), {
            pending: "Quitando registros del servicio en el usuario",
            success: "Registros quitados",
            error:
              "Error al quitar registros del servicio al usuario, inténtalo de nuevo",
          });
        }
    ...
    user && (
      <>
        <div className="margin-top-25">
          ...
          <p>
            Esta acción no se puede revertir, si quitas al usuario del servicio
            tendrás que volver a registrarlo.
            {selectedUser.role === "user" && enterprise.type === "driver" && (
              <>
                <br />
                <b>
                  El usuario ya no sera {DRIVER}, tendrá que volver a ser
                  registrado
                </b>
              </>
            )}
          ...
          >
            <TextField
              field={{
                ...
                validator: (d) =>
                  validateEmialWithComparison(d, selectedUser.data.email),
              }}
