  ...
  const approve = async () => {
    if (!reviewState.loading) {
      if (user && user.id && enterprise.id) {
        ...
        try {
          await toast.promise(aproveEnterpriseReq(enterprise.id, user.id), {
            pending: `Aprobando ${ENTERPRISE_TO_SPANISH[enterprise.type]}`,
            success: "Aprobado",
    ...
    if (!reviewState.loading) {
      if (user && user.id && enterprise.id) {
        ...
        try {
          ...
          await toast.promise(declineEnterpriseReq(enterprise, user.id), {
            pending: `Rechazando ${ENTERPRISE_TO_SPANISH[enterprise.type]}`,
            success: "Rechazado",
            error: `Error al rechazar ${
              ENTERPRISE_TO_SPANISH_WITH_DEFINITE_ARTICLE[enterprise.type]
            }`,
          });
          router.push(routeToRequestsToEditEnterpriseAsAdmin(enterprise.type));
        } catch (e) {
          console.log(e);
          setReviewState({
            ...reviewState,
      ...
      >
        ...
        {!enterprise.aproved && enterprise.active && (
          <p className="text | light margin-top-25">
            Se eliminaran los datos que ya <b>no seran necesarios</b> si
            rechazas la solicitud
          </p>
        )}
      </BaseFormWithTwoButtons>
    </section>
