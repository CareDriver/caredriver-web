      ...
      >
        {vehicle.type.mode.length === 1 && (
          <button
            ...
            onClick={() => transmisionAdder.addMissingTransmission(vehicleType)}
          >
            <Plus />
            Agregar transmisión{" "}
            {
              TRANSMITION_TO_SPANISH_V2[
                transmisionAdder.getMissTransmission(vehicle.type.mode)
              ]
            }
          </button>
        )}
        ...
        >
          <span className={"text | medium bold"}>Actualizar Licencia</span>
        </Link>
      </div>
    </div>
  );
