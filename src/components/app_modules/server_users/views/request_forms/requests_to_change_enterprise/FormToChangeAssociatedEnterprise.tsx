...
const FormToChangeAssociatedEnterprise: React.FC<Props> = ({
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      let hasRequestsSent = await toast.promise(
        ...
        {
          pending: "Verificando disponibilidad para enviar peticiones...",
          error: "error al verificar disponibilidad inténtalo de nuevo",
        },
      );
      if (hasRequestsSent) {
        toast.info("Tienes peticiones en revision, espera a que sean revisada");
        setFormState((prev) => ({ ...prev, loading: false }));
        return;
      }
    } catch (e) {
    ...
    let newEnterpriseBaseData: BaseSimpleEnterprise = form.newEnterprise.value
      .logoImgUrl
      ? {
    ...
    try {
      await toast.promise(
        sendRequestToChangeAssociatedEnterprise(requestData.id, requestData),
        {
  ...
  }, [checkingUserAuth, router, typeOfService, user]);
...
function isValidForm(form: Form): boolean {
  return (
    isValidTextField(form.reason) && isValidEntityDataField(form.newEnterprise)
  );
}
