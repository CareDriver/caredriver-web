...
const ListOfServicesPerfByUser: React.FC<Props> = ({
  ...
  const fetchDocuments = useCallback(
    ...
    [user?.id, COLLECTION_PATH, deadline.seconds, typeOfPerf],
  );
    ...
    if (typeOfService === "driver" || typeOfService === "tow") {
      return (
        <CardForServicePerfWithLocation
          ...
          service={service}
          fakeUserServerId={
            typeOfPerf === TypeOfServicePerformed.Served ? userId : undefined
          }
          key={key}
        />
      );
  ...
  return (
    ...
    >
      <h2 className="text | bold big margin-bottom-25 capitalize">
        Servicios <i className="text | bold big capitalize">{typeOfPerf}</i>
      </h2>
