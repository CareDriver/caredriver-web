...
const FormToDisableUserByDateByAdmin: React.FC<Props> = ({
    ...
    if (user.id) {
      if (isDisable) {
        ...
        try {
          await toast.promise(
            updateUser(user.id, {
              disabledUntil: Timestamp.fromDate(form.deadline.value),
            }),
            {
              pending: "Deshabilitando al usuario por fecha",
              success: "Usuario deshabilitado por fecha",
              error:
                "Error al deshabilitar al usuario por fecha, inténtalo de nuevo",
            },
          );
        } catch (e) {
          console.log(e);
  ...
  return (
    <div className={`form-sub-container | margin-top-50 max-width-40`}>
      ...
      <p>
        El usuario no podra usar la aplicacion mientras este desabilitando hasta
        ...
        {!isDisable && (
          <DateField
            field={{
              ...
              setter: (d) => setForm((prev) => ({ ...prev, deadline: d })),
