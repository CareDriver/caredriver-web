...
const FormToIncreaseUserBalanceByAdmin = ({
    ...
    if (user.id && adminUser.id) {
      try {
        ...
        const NEW_BALANCE: Price = {
          amount: parseFloat(form.newBalance.value) + OLD_BALANCE.amount,
          currency: "Bs. (BOB)",
        };
        ...
        if (form.reasonOfChange.type === "bankTransactionNumber") {
          balanceItem = {
            ...balanceItem,
            bankTransactionNumber: form.reasonOfChange.description.value,
          };
        } else {
          balanceItem = {
            ...balanceItem,
            modificationReason: form.reasonOfChange.description.value,
          };
        }

        await toast.promise(saveBalanceHistoryItem(DOC_ID, balanceItem), {
          pending: "Guardando justificativo",
          success: "Justificativo guardado",
          error:
            "Error al guardar tu justificativo, inténtalo de nuevo por favor",
        });
  ...
  }, [form.reasonOfChange.type, form.reasonOfChange.description.value]);
  ...
  return (
    <section className="profile-info-wrapper | margin-top-50 max-width-60">
      ...
      >
        <TextField
          field={{
            ...
            setter: (b) => setForm((prev) => ({ ...prev, newBalance: b })),
      ...
      <Popup isOpen={isOpenPopup} close={() => setOpenPopup(false)}>
        <div>
          <h2 className="text | big bold">Razón para incrementar el Saldo</h2>
          <p className="text | light">
            Escribe el número de transacción registrada en cuenta bancaria o la
            razón por la cual esta haciendo este incremento de saldo.
          </p>
        </div>
        <BaseForm
          content={{
