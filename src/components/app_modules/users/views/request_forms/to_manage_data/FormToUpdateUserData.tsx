...
const FormToUpdateUserData: React.FC<Props> = ({ user }) => {
  ...
  const changePhoto = async () => {
    ...
    let newPhoto: RefAttachment = await toast.promise(
      ...
      {
        pending: `${hadPhoto ? "Cambiando" : "Subiendo"} foto de perfil`,
      ...
      try {
        ...
        if (creditialsWasChanged(form, user)) {
          await toast.promise(editCredentials(), {
            ...
            error:
              "Error al cambiar credenciales, inténtalo de nuevo por favor",
          });
        }
      } catch (e) {
        setFormState((prev) => ({
    ...
    <div className="form-container | full-form margin-top-50 max-width-60">
      <div className="margin-bottom-15">
        ...
        <p className="text | light">
          ...
          <b>Tendrás que volver a iniciar sesión si cambias tus credenciales</b>
        </p>
      </div>
      <BaseForm
        content={{
      ...
      >
        <ImageUploader
          ...
          uploader={{
            image: form.photo,
            setImage: (p) => setForm((prev) => ({ ...prev, photo: p })),
          }}
        />
        <TextField
          field={{
            values: form.fullName,
            setter: (d) => setForm((prev) => ({ ...prev, fullName: d })),
        ...
        <PasswordField
          ...
          setter={(d) => setForm((prev) => ({ ...prev, password: d }))}
...
function creditialsWasChanged(form: Form, user: UserInterface): boolean {
  return (
    form.email.value !== user.email || !isNullOrEmptyText(form.password.value)
  );
}
...
function hasChanges(form: Form, user: UserInterface): boolean {
  return creditialsWasChanged(form, user) || personalDataWasChanged(form, user);
}
...
function isValidForm(form: Form): boolean {
  return (
    ...
    (isNullOrEmptyText(form.password.value) || isValidTextField(form.password))
  );
}
