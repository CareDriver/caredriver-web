  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!loading) {
      ...
      if (user.id && adminUser.id) {
        ...
        await toast.promise(updateUser(user.id, { role: form.newRole }), {
          pending: "Cambiando el rol del usuario",
          success: "Rol del usuario cambiado",
          error:
            "Error al cambiar el rol del usuario, inténtalo de nuevo por favor",
        });
    ...
    <section className="profile-info-wrapper | margin-top-50 max-width-60">
      ...
      <Popup isOpen={isOpenPopup} close={() => setOpenPopup(false)}>
        <div className="margin-bottom-25">
          <h2 className="text | big-medium bold">Razón del cambio de rol</h2>
          <p className="text | light">
            Escribe la razón por la cual estas haciendo este cambio en el
